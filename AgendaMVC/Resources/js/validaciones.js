...
function validarTelefonos(input, input2, minLength, maxLength){
    ...
    if(telFijo=="" && telCel ==""){
        input.focus()
        toastr.warning("Debe digitar por lo menos un teléfono de contatcto","Proceso detenido",{timeOut:2000})
        valid=false
    }
    if(telFijo  != ""){
        if (isNaN(telFijo)){
            input.focus()
            toastr.warning(`El teléfono solo puede contener números`,"Proceso detenido",{timeOut: 2000})
            valid=false
            ...
            input.focus()
            toastr.warning(`La cantidad de caracteres máxomos permitidos para el teléfono fijo son${maxLength}`,"proceso detenido",{timeOut: 2000})
            valid=false
        }
    } 
    ...
    if(telCel !=""){
        ...
        input2.focus()
        toastr.warning(`La cantidad de caracteres mínimos permitidos para el teléfono fijo son ${maxLength}`,"Proceso detenido",{timeOut:2000})
        valid=false
        }
    }
    return valid
